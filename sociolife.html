    ...
    <div id="ui-layer" class="relative flex flex-col justify-between p-4">
        ...
        <div id="hud-panel" class="panel rounded-lg p-4 w-full max-w-7xl mx-auto flex flex-wrap gap-6 justify-between items-center shadow-lg z-10 relative">
            <div>
                ...
                <div class="text-[10px] text-red-400 font-bold hidden" id="crisis-alert">⚠️ EXTINCTION RISK - REPRO RATE UP ⚠️</div>
                <div class="text-[10px] text-green-400 font-bold hidden" id="tool-alert">⚠️ INFECTION TOOL ACTIVE - CLICK TO DROP ⚠️</div>
                <div class="text-[10px] text-green-300 font-bold" id="status-alert">✨ Life Signs Healthy ✨</div>
            </div>
            ...
            <div class="flex gap-2 items-center">
                <button onclick="sim.setTool('infect')" id="btn-infect" class="px-3 py-1 bg-green-900/80 hover:bg-green-700 rounded text-xs border border-green-500 transition text-white font-bold">Infect Tool</button>
                <button onclick="sim.setTool('monster')" id="btn-monster" class="px-3 py-1 bg-purple-900/80 hover:bg-purple-700 rounded text-xs border border-purple-500 transition text-white font-bold">Spawn Monster</button>
        ...
        <div id="settings-modal" class="panel absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-5 rounded-xl hidden shadow-2xl w-80 md:w-96 z-50 border border-blue-900/50">
            ...
            <div class="mt-4 pt-2 border-t border-gray-700 flex justify-between items-center gap-2">
                ...
                <div class="flex gap-2">
                    <button onclick="sim.copySettingsToClipboard()" class="text-xs text-blue-400 hover:text-blue-300 px-2 py-1 rounded border border-gray-700 hover:border-blue-500 transition">📋 Copy Settings</button>
            ...
            <div id="inspector" class="panel rounded-lg p-4 w-80 hidden shadow-xl border-l-4 border-l-blue-500">
                <div class="flex justify-between items-start border-b border-gray-700 pb-2 mb-2">
                    <div>
                        ...
                        <div class="text-[10px] font-bold text-orange-400" id="ins-hero" style="display:none;">★ HERO ★</div>
                    </div>
        ...
        */
            ...
            update() {
                ...
                if (typeof sim !== 'undefined' && sim.frame % 60 === 0 && !isInBabyBoom) { // Check every 60 frames, PAUSED during baby boom
                    ...
                    if (tribe) {
                        ...
                        // Positive score → grow, Negative score → decay
        ...
        class TribeManager {
            ...
            declareWar(tribe1Id, tribe2Id) {
                ...
                if (typeof sim !== 'undefined') {
                    sim.setStatusMessage(`⚔️ WAR DECLARED: Tribe ${tribe1Id} vs Tribe ${tribe2Id}`, 300); // Show for 300 frames (~5 seconds)
                }
            ...
            endWar(tribe1Id, tribe2Id, peaceTreaty = false) {
                ...
                // Update HUD status message
                if (typeof sim !== 'undefined') {
                    sim.setStatusMessage(`🕊️ WAR ENDED: Tribe ${tribe1Id} and Tribe ${tribe2Id} made peace`, 300); // Show for 300 frames
                }
                ...
                for (let otherId in this.tribes) {
                        ...
                        if (recentProfit > 0) {
                            ...
                            if (nest && otherNest) {
                                ...
                                if (!canTrade) {
                                    ...
                                    // UNPROFITABLE BOND BREAKS → DISLIKE!
                    ...
                    if (dipBond > 0) {
                        ...
                        if (framesSinceLastDiplomacy < 1000 || diplomaticValue > 0) {
                            ...
                            // DIPLOMATIC BOND BREAKS → DISLIKE!
        ...
        class Agent {
            constructor(x, y, dna, parents, tribeId) {
                ...
                this.mass = this.baseRadius * (this.health / 100); // Mass = size × health factor
                this.maxForce = 0.3; 
            ...
            update(resources, agents, doExpensiveOps = true) {
                ...
                // - Unhealthy (≤40%): death rate is 100% of normal (full death rate)
                ...
                if (this.role === 'Botanist') this.energy += 0.1; 
                if (this.role === 'Botanist' && this.energy > 150 && Math.random() < 0.02) {
                    sim.spawnResource(null, this.pos.x + (Math.random()*20-10), this.pos.y + (Math.random()*20-10), 'SEED', false, this.tribeId);
                    this.energy -= 30; 
                }
                
                // BOTANIST FERTILIZATION TRAIL - plants seeds and fertilizes as they move
                if (this.role === 'Botanist' && Math.random() < CONFIG.botanistFertilizeRate) {
                    // Fertilize nearby food/seed resources (extend their life)
                    let nearbyFood = resources.filter(r => 
                        (r.type === 'FOOD' || r.type === 'SEED') && 
                    ...
                    // Plant new seeds occasionally
                    if (Math.random() < 0.3 && this.energy > 30) {
                        sim.spawnResource(null, this.pos.x + (Math.random()*30-15), this.pos.y + (Math.random()*30-15), 'SEED', false, this.tribeId);
                        this.energy -= 5;
            ...
            interact(resources, agents, doExpensiveOps = true) {
                ...
                // BOTANIST GARDENING
                if (this.role === 'Botanist' && this.carrying && this.carrying.type === 'SEED' && myNest) {
                    // Plant seed near nest
                    forces.base = this.seek(myNest.pos).mult(1.5);
                    this.state = "Planting";
                    active = true;
                    if (distToNest < 80 && distToNest > 30) {
                        // Plant here
                        this.carrying.active = false;
                        sim.spawnResource(null, this.pos.x, this.pos.y, 'FOOD', false, this.tribeId); // Instant grow for now, or could spawn new SEED
                        this.carrying = null;
                        sim.particles.push(new Particle(this.pos.x, this.pos.y, '#4ade80', 'spark'));
                    }
                }
                ...
                // FARMER BEHAVIOR
                if (this.role === 'Farmer') {
                    // Spawn grazers based on health
                    let healthRatio = this.health / this.maxHealth;
                    if (healthRatio > CONFIG.farmerSpawnGrazerHealthThreshold && 
                        Math.random() < CONFIG.farmerSpawnGrazerRate && 
                        this.energy > 50) {
                ...
                if (this.role === 'Miner') {
                    ...
                    if (this.carrying && myNest) {
                        ...
                        if (distToNest > myNest.radius) {
                            ...
                            this.carrying.carrier = null;
                            this.carrying = null;
                            myNest.wealth += resourceValue * 5; // Miners contribute 5× resource value (MASSIVE INCREASE!)
                    ...
                    if (tradePartners.length > 0 && this.carrying) {
                        ...
                        if (d > targetNest.radius + 30) {
                            ...
                            if (Math.random() < CONFIG.merchantTradeFrequency && this.carrying) {
                                ...
                                let myProfit = baseValue * 15 * myNegotiationPower * strengthRatio; // Merchant profit (5× increase!)
                                let homeProfit = baseValue * 10 * strengthRatio; // Home nest profit (5× increase!)
                                let theirCost = baseValue * 5 / strengthRatio; // They pay (but increased)
                                let theirBenefit = baseValue * 10; // They benefit greatly! (6.7× increase!)
            ...
            checkMating(allAgents) {
                ...
                if (this.isPredator) {
                    ...
                    let reproRate = 0.002 * (sim.dynamicBirthRateMult || 1.0); // 0.2% per frame (4× increase!)
                ...
                // Base reproduction rate (INCREASED 5× for stronger tribes!)
        ...
        class Simulation {
            ...
            setTool(tool) {
                ...
                let btnMonster = document.getElementById('btn-monster'); // ADDED ID TO MONSTER BUTTON
                let alertBox = document.getElementById('tool-alert');
                
                // Clear classes
                btnInfect.classList.remove('active-tool');
                if(btnMonster) btnMonster.classList.remove('active-tool-monster'); // Custom class for purple
                alertBox.classList.add('hidden');

                if (this.actionMode === 'infect') {
                    btnInfect.classList.add('active-tool');
                    alertBox.textContent = "⚠️ INFECTION TOOL ACTIVE - CLICK TO DROP ⚠️";
                    alertBox.classList.remove('hidden');
                } else if (this.actionMode === 'monster') {
                    if(btnMonster) btnMonster.classList.add('active-tool-monster');
                    alertBox.textContent = "⚠️ MONSTER SPAWN ACTIVE - CLICK LOCATION ⚠️";
                    alertBox.classList.remove('hidden');
                }
            }
            ...
            input(e) {
                ...
                if (this.actionMode === 'infect') {
                    ...
                    this.setStatusMessage(`🌋 CATACLYSM: Plague unleashed at (${Math.floor(x)}, ${Math.floor(y)})`, 300); // Show for 300 frames
                    ...
                    if (this.dynamicBirthRateMult > 1.5) {
                        ...
                        if (!toolAlertBox || toolAlertBox.classList.contains('hidden')) {
                            ...
                            if (this.statusMessage && this.frame < this.statusMessageExpiry) {
                                ...
                                statusAlertBox.textContent = "✨ Life Signs Healthy ✨";
                                statusAlertBox.className = 'text-[10px] text-green-300 font-bold';
                            }
                        }
                    }
            ...
            loop() {
                ...
                this.plagues.forEach(p => {
                    this.agents.forEach(a => {
                        if (Vector.dist(a.pos, p.pos) < p.radius) a.disease = 200; // Reduced from 500 (60% reduction) - less severe
                    });
                });
                this.plagues = this.plagues.filter(p => p.life > 0);
                ...
                if (this.frame % 3000 === 0) { // Reduced Frequency
                    ...
                    if (Math.random() < 0.5) { // Reduced Chance
                        ...
                        if (disaster < 0.3) {
                            ...
                            this.setStatusMessage(`🌵 CATACLYSM: Drought - all food resources removed`, 300);
                        } else if (disaster < 0.6) {
                            // Boom
                            for(let i=0; i<20; i++) this.spawnResource(null, null, null, 'TREASURE');
                            this.setStatusMessage(`💰 CATACLYSM: Treasure boom - resources scattered`, 300);
                        } else {
                            // Pestilence
                            let plagueX = Math.random()*this.width;
                            let plagueY = Math.random()*this.height;
                            this.plagues.push(new Plague(plagueX, plagueY));
                            this.setStatusMessage(`🌋 CATACLYSM: Plague unleashed at (${Math.floor(plagueX)}, ${Math.floor(plagueY)})`, 300);
                        }
                    }
